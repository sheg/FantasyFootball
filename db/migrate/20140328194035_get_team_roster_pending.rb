class GetTeamRosterPending < ActiveRecord::Migration
  def up
      ...
      BEGIN
        ...
        declare TransCursor cursor for
          ...
          where
            ...
            and tt.transaction_status_id in (select * from temp_status_ids)
          order by transaction_date, tt.id
        ;

        DECLARE CONTINUE HANDLER FOR NOT FOUND SET no_more_rows = TRUE;

        drop table if exists temp_status_ids;
        create temporary table temp_status_ids select 1;
        if IncludePending then insert into temp_status_ids values (2); end if;
